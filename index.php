...
<body class="align">
    ...
    <div class="grid" style="margin-bottom: 80px" ;>
        ...
        <h1 style="text-align: center; color: #fff;">Ingrese el Usuario </h1>
        ...
        if (isset($_POST['login'])) {
            ...
            if ($txtuser == "" || $txtpassword == "") {
                ...
                include_once("php/includes/validar.php");
                $puntero = new ingreso();
                $puntero->mostrar($txtuser, $txtpassword);

                //validacion de ingreso con usuarios para evitar inyecciones sql if(isset($_SESSION['usuario']))
                if (isset($_SESSION['usuario'])) {
                    $duda = $_SESSION['usuario'];
                    echo "<script type='text/javascript'> document.location = 'view/inicio.php'; </script>";
                    echo "<script>alert('$duda');</script>";
                } else {

                    echo "<script>alert('Credenciales Denegadas');</script>";
